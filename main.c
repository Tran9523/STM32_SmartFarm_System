...
int Main(void)
{
    ...
    while (1)
    {
        ...
        if (water_level < 1500) {
            ...
            if (!is_watering && (System_Tick - last_watering_time >= 10000)) {  // 급수 주기 변경
                is_watering = 1;
                water_start_time = System_Tick;
            }

            if (is_watering) {
                is_active = 1; // [추가] 급수 펌프 동작 중 마킹
                Pump_Set(1, 80);    // DC 파워 %

                if (System_Tick - last_hose_sweep >= 30) {  // 호스 스윙 속도 변경 - 높을 수록 천천히
                    hose_angle += (hose_dir * 5);
                    if (hose_angle >= 90 || hose_angle <= 0) hose_dir *= -1;
                    Servo_Hose_Set(hose_angle);
                    last_hose_sweep = System_Tick;
                }

                if (System_Tick - water_start_time >= 3000) {   // 급수량(시간) 변경
                    is_watering = 0;
                    Pump_Set(0, 0);
        ...
        // ----------------------------------------------------
        // [4번 기능] 조도 센서 필터링 및 환경 LED/블라인드 제어
        // ----------------------------------------------------
        ...
        // LED가 모두 꺼진(0) 상태에서 빛이 더 강해지면 블라인드가 작동합니다.
